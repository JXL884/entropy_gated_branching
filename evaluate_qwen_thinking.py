#!/usr/bin/env python3
"""Helper script to run eval.py across qwen_thinking outputs."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Batch runner for eval.py over qwen_thinking outputs."
    )
    default_root = get_output_root() / "qwen_thinking"
    parser.add_argument(
        "--root",
        type=Path,
        default=default_root,
        help="Root directory containing qwen_thinking outputs (default: qwen_thinking under the output root).",
    )
    parser.add_argument(
        "--mode",
        choices=["parser", "llm"],
    ...
    parser.add_argument(
        "--search-modes",
        nargs="+",
        help="Optional list of search-mode directory names to evaluate (e.g. beam, confidence, normal).",
    )
    parser.add_argument(
        "--models",
        nargs="+",
        help="Optional list of model directory names to evaluate (matches folder names under each search mode).",
    )
    parser.add_argument(
        "--datasets",
        nargs="+",
        help="Optional list of dataset names to evaluate (leaf folder names like 'math', 'l2').",
    )
    parser.add_argument(
        "--llm-model",
...
def iter_result_dirs(
    root: Path,
    search_modes: Iterable[str] | None,
    models: Iterable[str] | None,
    datasets: Iterable[str] | None,
) -> Iterable[Tuple[str, str, str, str, Path, Path]]:
    """Yield dataset directories under ``root`` including search-mode grouping."""
    if not root.exists():
        raise FileNotFoundError(f"Results root '{root}' does not exist.")

    search_mode_filter = set(search_modes) if search_modes else None
    model_filter = set(models) if models else None
    dataset_filter = set(datasets) if datasets else None

    for search_dir in sorted(p for p in root.iterdir() if p.is_dir()):
        search_mode = search_dir.name
        if search_mode_filter and search_mode not in search_mode_filter:
            continue

        for model_dir in sorted(p for p in search_dir.iterdir() if p.is_dir()):
            ...
            for dataset_dir in _iter_dataset_dirs(model_dir):
                ...
                has_json = any(dataset_dir.glob("*.json"))
                if not has_json:
                    print(f"[skip] {search_mode}/{model_name}/{dataset_name}: no JSON files found.")
                    continue
...
def main() -> int:
    ...
    for search_mode, model_name, dataset_name, dataset_key, dataset_dir, rel_path in iter_result_dirs(
        root, args.search_modes, args.models, args.datasets
    ):
        cmd = build_command(
            eval_py,
            dataset_dir,
        ...
        pretty_target = f"{search_mode}/{model_name}/{dataset_name}"
