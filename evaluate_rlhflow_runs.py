#!/usr/bin/env python3
"""Helper script to run eval.py across rlhflow run outputs."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Batch runner for eval.py over rlhflow run outputs."
    )
    default_root = get_output_root() / "rlhflow_runs"
    parser.add_argument(
        "--root",
        type=Path,
        default=default_root,
        help="Root directory containing rlhflow outputs (default: rlhflow_runs under the output root).",
    )
    parser.add_argument(
        "--mode",
        choices=["parser", "llm"],
    ...
    parser.add_argument(
        "--search-modes",
        nargs="+",
        help="Optional list of search-mode directory names to evaluate (e.g. beam, confidence).",
    )
    parser.add_argument(
        "--models",
        nargs="+",
